...
__all__ = ['StringReasonType', 'INSTRUCTIONS', 'def_refparam', 'def_refmodeparam',
           ...
           'get_refmodeparam', 'set_refcondmodeparam', 'get_refcondmodeparam', 'crc16',
           'Sum_Right', 'singleton', 'PingIP', 'HexToString', 'log', 'tryException',
           'decimal_default','getRefParamDict','getRefModeParamDict','getRefCondModeParamDict']
...
INSTRUCTIONS = Enum(['LED_AUTO', 'LED_ON','LED_OFF','LED_URGENT',
                     'ON', 'OFF',
                     'gaosu', 'zhongsu', 'disu', 'tingzhi', 'fakai', 'faguan',
                     'zhileng', 'zhire', 'reshui', 'zl&rs', 'zr&rs',
                     'tongfeng', 'zidong', 'jjzhire', 'kszhire', 'jire'
                     ])
...
def getRefModeParamDict():
    return RefModeParam

def getRefCondModeParamDict():
    return RefCondModeParam
...
def log(level, *args, **kwargs):
    def inner(func):
        def wrapper(*args, **kwargs):
            print level, 'before calling ', func.__name__, 'args :', args, 'kwargs:', kwargs
            func(*args, **kwargs)
            print level, 'end calling ', func.__name__
        return wrapper
    return inner

def tryException(func):
    def wrapper(*args, **kwargs):
        try:
            return func(*args, **kwargs)
        except Exception as e:
            print datetime.now(), 'calling', func.__name__, 'args :', args, 'kwargs :', kwargs, 'Error : ', e
    return wrapper
...
def Sum_Right(array):
    ...
    for i in range(len(array) -1):
        check_sum += array[i]
        check_sum &= 0xff
    if check_sum == array[len(array) -1]:
        return True
    else:
        return False
